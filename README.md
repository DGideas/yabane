...
**Reasoning replay limitation (PROXY-47):** Responses history sent to a Chat Completions or Anthropic Endpoint keeps supported messages and function calls/results, but omits `reasoning` items, including summaries, plaintext reasoning, and encrypted Provider state. The generic adapter cannot infer support for non-standard fields such as `reasoning_content`, and never substitutes reasoning for visible assistant text. This lets supported conversation history continue after changing Endpoint protocol, but is not lossless reasoning replay; callers requiring that continuity must keep a native Responses path. Unsupported item types (including unresolved `item_reference` items), invalid message roles, and reasoning-only input fail locally with `400` instead of producing malformed Provider messages. Native Responses forwarding is unchanged.

**Parallel tool-call grouping (PROXY-51):** A parallel batch — several Responses `function_call` items produced by one assistant turn — replays to Chat Completions as a single assistant message carrying all of its calls, so each call is answered by the tool messages that immediately follow the assistant message, as Chat Providers require. A call whose result already follows it stays a separate turn. In the reverse direction consecutive Chat tool results that answer one batch replay as the single Anthropic user message with all their `tool_result` blocks. Call IDs, names, arguments, and result content are passed through unchanged.
