...
trap cleanup EXIT
cd "$work"
TURNSTILE_SECRET="${TURNSTILE_TEST_SECRET:-1x0000000000000000000000000000000AA}" YABANE_ADDR="127.0.0.1:$port" "$binary" >server.log 2>&1 & pid=$!
...
wait "$pid"
pid=
[[ $(wc -l < data/activity.jsonl | tr -d ' ') -eq 11 ]]
