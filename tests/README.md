...
- `tests/reasoning-conversion.mjs` — Responses history containing reasoning reaches a Chat-only Endpoint with valid ordered messages and tool images in JSON and SSE modes; a parallel tool-call batch, including interleaved assistant text, replays as one answerable assistant message under a strict Provider check, and unsupported items/server-side history fail locally without a Provider request (PROXY-47 / PROXY-09 / PROXY-51 / PROXY-53 / PROXY-56).
- `tests/stream-semantics.mjs` — JSON/SSE/aggregation retain token-limit termination and final usage, tool events agree with terminal output, empty text deltas do not insert assistant messages into tool-only turns replayed to strict Responses and Chat Providers, mixed-newline frames convert correctly, and native requests/responses (including empty deltas) stay unchanged. It also checks actual Provider requests for Chat part conversion and omission of foreign reasoning, strict Anthropic block assembly, terminal-only content reconciliation, and rejection of divergent/unfinished output (PROXY-48 / PROXY-49 / PROXY-50 / PROXY-52 / PROXY-54 / PROXY-55 / PROXY-57 / PROXY-58 / PROXY-59 / PROXY-60 / PROXY-61 / PROXY-62 / PROXY-63).
...
Request/JSON regression cases are in `src/protocol/regression_tests.rs`; streaming regressions are in `src/protocol_stream/regression_tests.rs`. They cover exact argument strings, tool-batch boundaries, parameter shapes, MIME-preserving document round trips, bounded cumulative-content reconciliation and strict block identities, alongside the process-level fixtures above.
