...
* After setup, unauthenticated visitors see a responsive, brand-faceted login screen with a high-contrast form panel and can sign in with username or email plus password; first-use administrator registration shares the same visual system, and decorative artwork is inert and respects reduced-motion preferences.
...
* The authenticated Home page summarizes 24-hour requests and token usage, links to configured providers, and automatically refreshes its statistics while the page and browser tab are visible.
