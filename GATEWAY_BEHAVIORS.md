...
* The Model routing page explains that rules bind matching models to a specific Endpoint and upstream credential, shows exact and prefix examples, and makes clear that unmatched models retain default Endpoint credential selection; each configured rule visually separates its public match, upstream model, Endpoint, credential, active status, and normalized traffic percentage instead of presenting the destination as an undifferentiated text string.
* Missing configuration files produce empty/default configuration; malformed or unreadable configuration, including duplicate Provider, Endpoint-within-Provider, or key-within-Endpoint identities, fails startup visibly instead of silently overwriting or ambiguously addressing resources.
...
* Gateway API-key secrets are persisted so the administrator can reveal or copy them later; their SHA-256 hashes are also retained and compared in constant time for request authentication.
...
* Activity can be filtered by time range and Provider, automatically refreshes statistics and requests at a bounded cadence while the page and browser tab are visible, can be refreshed on demand, and can be switched between the visual Overview and a searchable Request explorer with status filtering; Overview fetches only its small recent-request sample, the explorer loads its larger sample on demand and bounds rendered rows, and Provider filtering is performed server-side; the explorer adapts its usable list height to the viewport, emphasizes model, route, status, latency, and token output while retaining secondary metadata, and selecting a request opens a responsive detail dialog with its route, protocol, status, timing breakdown, token usage, derived output throughput, and cost while reiterating that prompt and response content are not retained.
